  ...
  return (
    <div className="opacity-50">
      <div className="mb-1 flex items-baseline gap-2.5">
        <span className="hidden sm:inline text-xs text-amber-dim">d---------</span>
        <span className="text-[15px] tracking-[0.5px] text-amber-dim">
          [LOCKED]/
        </span>
      </div>
      <div className="text-[13px] leading-normal text-amber-dim">
        Permission denied. Security clearance required.
      </div>
      <span className="mt-0.5 inline-block text-[11px] text-amber-dim">
        → ACCESS RESTRICTED
      </span>
    </div>
  );
    ...
    <section className="mb-12">
      ...
      <div className="overflow-hidden rounded-[2px] border border-amber-dim">
        {projects.map((project, index) => (
          <div
            key={project.name}
            className={`p-3 sm:grid sm:grid-cols-[1fr_auto] sm:items-start sm:gap-2 sm:p-[14px_18px] transition-colors ${
              project.classified ? "" : "hover:bg-amber-glow"
            } ${
          ...
          >
            ...
            <div className="mt-2 flex flex-wrap gap-1 sm:mt-0 sm:flex-col sm:items-end">
              {project.tags.map((tag) => (
                <span
                  key={tag}
                  className={`whitespace-nowrap rounded-[1px] border px-[7px] py-px text-[11px] ${
                    project.classified
                      ? "border-amber-dim/50 text-amber-dim/50"
                      : "border-amber-dim text-amber-dim"
                  }`}
                >
                  {tag}
                </span>
